function [] = saveDenoisedGrayPic(events,eventDelta,saveDir)
%function [] = saveDenoisedGrayPic( events,eventDelta,saveDir )
%
% Save denoised gray pics under the floder 'denoisedGrayPics' in 
...
narginchk(3,3);
saveDir=fullfile(saveDir,'denoisedGrayPics');
dirChk(saveDir)

% Variable set-up
global X Y
len=length(events.x);
denoisedGrayImg=zeros(Y,X);
heatMap=zeros(Y,X);
...
fprintf('Start saving denoised gray images...\n')
while (imgIndex<=stopNum)
    ...
    eventsAdcBuffer(:)=events.y(startEventIndex:stopEventIndex);
    eventIndex=1;
    while eventIndex<=eventDelta
        row=eventsYBuffer(eventIndex);
        col=eventsXBuffer(eventIndex);
        denoisedGrayImg(row,col)=eventsAdcBuffer(eventIndex);
        heatMap(row,col)=heatMap(row,col)+1;
        eventIndex=eventIndex+1;
    end
    weightMap=getWeightMap(heatMap);
    denoisedGrayImg=denoisedGrayImg.*weightMap;
    imwrite(denoisedGrayImg,fullfile(saveDir,sprintf('%s.jpg',num2str(imgIndex))));
    denoisedGrayImg(:)=0;
    heatMap(:)=0;
    imgIndex=imgIndex+1;
end
fprintf('Finished denoised gray images.\n')
end
