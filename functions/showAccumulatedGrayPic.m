function [] = showAccumulatedGrayPic(events,eventDelta,displayTime)
%function [] = showAcculatedGrayPic(events,eventDelta,displayTime)
%
% Show accumulated gray pic which is formed by accumulating a number of events.
%
% Takes in:
%     'events'
...
while (imgIndex<=stopNum)
    ...
    imshow(accumulatedGrayImg,[])
    title(sprintf('Accumulated Gray Pic %s',num2str(imgIndex)))
    pause(displayTime)
    imgIndex=imgIndex+1;
end
close
