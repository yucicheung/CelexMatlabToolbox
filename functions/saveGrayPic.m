function [] = saveGrayPic(events,eventDelta,saveDir)
%function [] = saveGrayPic( events,eventDelta,saveDir )
%
% Save gray pics, which will be saved under the folder 'grayPics' in 
...
%     'saveDir'
%         The directory to place the folder for storing gray pics. 
%     
...
narginchk(3,3);
saveDir=fullfile(saveDir,'grayPics');
dirChk(saveDir)
...
fprintf('Start saving gray images...\n')
while (imgIndex<=stopNum)
    ...
    while eventIndex<=eventDelta
        grayImg(eventsYBuffer(eventIndex),eventsXBuffer(eventIndex))=...
            eventsAdcBuffer(eventIndex);
        eventIndex=eventIndex+1;
    end
    normalizedGrayImg=normalizePic(grayImg);
    picPath=fullfile(saveDir,sprintf('%s%s',num2str(imgIndex),'.jpg'));
    imwrite(normalizedGrayImg,picPath)
    grayImg(:)=0;
    imgIndex=imgIndex+1;
end
fprintf('Finished saving gray images.\n')
end
% ----------------------------------------------------------------------- %
