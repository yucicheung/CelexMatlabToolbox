function [] = showGrayPic(events,eventDelta,displayTime)
%function [] = showGrayPic(events,eventDelta,displayTime)
%
% Show gray pic which is formed by accumulating a number of events.
%
% Takes in:
%     'events'
...
while (imgIndex<=stopNum)
    ...
    imshow(grayImg,[])
    title(sprintf('Gray Pic %s',num2str(imgIndex)))
    pause(displayTime)
    grayImg(:)=0;
    imgIndex=imgIndex+1;
end
