function []=saveDenoisedBinaryComparison(events,eventDelta,saveDir)
%function [] = saveDenoisedBinaryComparison( events,eventDelta,saveDir )
%
% Save concatenated pic of the binary pic and its denoised-version pic 
% under the floder 'denoisedBinaryComparison' in specified directory 'saveDir'.
% 
% Takes in:
%     'events'
...
narginchk(3,3);
saveDir=fullfile(saveDir,'denoisedBinaryComparison');
dirChk(saveDir)
...
fprintf('Start saving DenoisedBinaryComparison images...\n')
while (imgIndex<=stopNum)
    ...
    binaryImg(:)=0;
    heatMap(:)=0;
    imwrite(concatImg,fullfile(saveDir,sprintf('%s.jpg',num2str(imgIndex))));
    imgIndex=imgIndex+1;
end
fprintf('Finished saving DenoisedBinaryComparison images.\n')
end
